<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:89:"D:\phpstudy\PHPTutorial\WWW\sign\public/../application/admin\view\panel\menus_manage.html";i:1572597443;s:72:"D:\phpstudy\PHPTutorial\WWW\sign\application\admin\view\common\head.html";i:1570699778;s:72:"D:\phpstudy\PHPTutorial\WWW\sign\application\admin\view\common\foot.html";i:1572573889;}*/ ?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8">
...
<span>栏目管理</span>  
<?php if($pid >0): ?>
 <button class="layui-btn layui-btn-primary layui-btn-sm" style="float:right;margin:5px;top:0px;" onclick="back(<?php echo $backid; ?>); return false;">返回上一级</button>
<?php endif; ?> 
<div></div>  
</div>
<form class="layui-form">
 ...
 <table class="layui-table">
    ...
    <tbody>
    <?php if(is_array($data['lists']) || $data['lists'] instanceof \think\Collection || $data['lists'] instanceof \think\Paginator): $key = 0; $__LIST__ = $data['lists'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;if($permission == 4): ?>
        <tr>
        <td><?php echo $key; ?></td>     
        <td>
          <input type="text" class="layui-input" name="ords[<?php echo $vo['mid']; ?>]" value="<?php echo $vo['ord']; ?>"><br>
        </td>
        <td>
          <input type="text" class="layui-input" name="titles[<?php echo $vo['mid']; ?>]" value="<?php echo $vo['title']; ?>" ><br>
        </td>
        <td>
          <input type="text" class="layui-input" name="controllers[<?php echo $vo['mid']; ?>]" value="<?php echo $vo['controller']; ?>" ><br>
        </td>
        <td>
          <input type="text" class="layui-input" name="methods[<?php echo $vo['mid']; ?>]" value="<?php echo $vo['method']; ?>" ><br>
        </td>    
        <td>
          <input type="checkbox" lay-skin="primary" name="status[<?php echo $vo['mid']; ?>]" title="禁用" <?php echo !empty($vo['status'])?'checked' :''; ?> value=1 ><br>
        </td>
        <td><button class="layui-btn layui-btn-xs" onclick="child(<?php echo $vo['mid']; ?>);return false;">展开</button></td>     
      </tr>
      <?php else: ?>         
      <tr>
        <td><?php echo $key; ?></td>     
        <td>
          <input type="text" class="layui-input" name="ords[<?php echo $vo['mid']; ?>]" value="<?php echo $vo['ord']; ?>" readonly><br>
        </td>
        <td>
          <input type="text" class="layui-input" name="titles[<?php echo $vo['mid']; ?>]" value="<?php echo $vo['title']; ?>"  readonly><br>
        </td>
        <td>
          <input type="text" class="layui-input" name="controllers[<?php echo $vo['mid']; ?>]" value="<?php echo $vo['controller']; ?>"  readonly><br>
        </td>
        <td>
          <input type="text" class="layui-input" name="methods[<?php echo $vo['mid']; ?>]" value="<?php echo $vo['method']; ?>"  readonly><br>
        </td>    
        <td>
          <input type="checkbox" lay-skin="primary" name="status[<?php echo $vo['mid']; ?>]" title="禁用" <?php echo !empty($vo['status'])?'checked' :''; ?> value=1  readonly><br>
        </td>
        <td><button class="layui-btn layui-btn-xs" onclick="child(<?php echo $vo['mid']; ?>);return false;">展开</button></td>     
      </tr>
