<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:91:"D:\phpstudy\PHPTutorial\WWW\sign\public/../application/admin\view\meeting\label_assign.html";i:1572574412;s:72:"D:\phpstudy\PHPTutorial\WWW\sign\application\admin\view\common\head.html";i:1570699778;s:72:"D:\phpstudy\PHPTutorial\WWW\sign\application\admin\view\common\foot.html";i:1572840312;}*/ ?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8">
...
<button class="layui-btn" type="button" style="margin-left:150px;" onclick="searchs()"><i class="layui-icon">&#xe615;</i>搜索</button>
  ...
  <table class="layui-table">
    ...
    <?php if(is_array($data['lists']) || $data['lists'] instanceof \think\Collection || $data['lists'] instanceof \think\Paginator): $i = 0; $__LIST__ = $data['lists'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
     <tr>
      <td>
       <?php if(empty($vo['exists'])): ?>
       <input type="checkbox" class="checkAll" name="checkAll[]" value="<?php echo $vo['Account']; ?>,<?php echo $vo['ClassNum']; ?>" onclick="selectAll(this)" >
       <?php else: ?>     
...
<script type="text/javascript">
   ...
   function searchs(curpage){
    ...
    var url ="<?php echo url('Meeting/label_assign'); ?>?labelid="+labelid+"&page="+curpage;
    console.log(labelid);
    if(seek){
      //将搜索到的内容拼接至分页后面
      url+='&seek='+seek;
  ...
  /*console.log(arr);
   return ;*/
     $.post("<?php echo url('Meeting/save_invite'); ?>",{'checkAll':arr,'id':id,'labelid':labelid},function(res){       
      if(res.code>0){
        layer.alert(res.msg,{icon:2});
      }else{
            layer.msg(res.msg);
