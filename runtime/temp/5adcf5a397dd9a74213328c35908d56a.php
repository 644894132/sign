<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:87:"D:\phpstudy\PHPTutorial\WWW\sign\public/../application/admin\view\menus\menu_lists.html";i:1570499099;s:72:"D:\phpstudy\PHPTutorial\WWW\sign\application\admin\view\common\head.html";i:1569669079;s:72:"D:\phpstudy\PHPTutorial\WWW\sign\application\admin\view\common\foot.html";i:1569665956;}*/ ?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8">
   ...
   <table class="layui-table">
      ...
      <tbody>
        ...
        <?php if(is_array($data['lists']) || $data['lists'] instanceof \think\Collection || $data['lists'] instanceof \think\Paginator): $i = 0; $__LIST__ = $data['lists'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
          ...
          <td><input type="text" class="layui-input" name="title" value="<?php echo $vo['title']; ?>"></td>
          <td><input type="text" class="layui-input" name="controller" value="<?php echo $vo['controller']; ?>"></td>
          <td><input type="text" class="layui-input" name="method" value="<?php echo $vo['method']; ?>"></td>
          <td><input type="checkbox" lay-skin="primary" name="ishidden" title="隐藏" value=1></td>
          <td><input type="checkbox" lay-skin="primary" name="status" title="禁用" value=1></td>
        <?php endforeach; endif; else: echo "" ;endif; ?>
          ...
          <tr>
            ...
            <td><input type="text" class="layui-input" name="" ></td>
            <td><input type="text" class="layui-input" name="" ></td>
            <td><input type="checkbox" lay-skin="primary" name="" title="隐藏" value=1></td>
            <td><input type="checkbox" lay-skin="primary" name="" title="禁用" value=1></td>
          </tr>
      </tbody>
   </table>
</form> 
...
<script type="text/javascript">
    function save(){
       //获取数据
      var mid =parseInt($('input[name="mid"]').val());
      var ord =$.trim($('input[name="ord"]').val());
      var title =$.trim($('input[name="title"]').val());
      var controller =$.trim($('input[name="controller"]').val());
      var method =$.trim($('input[name="method"]').val());

      $.post('/admin.php/admin/Menus/save',{'mid':mid,'title':title,'controller':controller,'method':method},function(res){
        if(res.msg>0){
          layer.alert(res.msg,{icon:2});
        }else{
          layer.msg(res.msg);
