<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:92:"G:\PHPstudayV8\phpstudy_pro\WWW\sign\public/../application/admin\view\panel\role_manage.html";i:1572573663;s:76:"G:\PHPstudayV8\phpstudy_pro\WWW\sign\application\admin\view\common\head.html";i:1570699779;s:76:"G:\PHPstudayV8\phpstudy_pro\WWW\sign\application\admin\view\common\foot.html";i:1572849076;}*/ ?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8">
...
<table class="layui-table">
 ...
 <?php if(is_array($data['lists']) || $data['lists'] instanceof \think\Collection || $data['lists'] instanceof \think\Paginator): $key = 0; $__LIST__ = $data['lists'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?>
    <tr>
      ...
      <td>
        <button class="layui-btn layui-btn-normal" onclick="add_role(<?php echo $vo['id']; ?>)">编辑</button>
    ...
    function add_role(id){
      layer.open({
       type: 2,
        ...
        shade: 0.3,
        area: ['800px', '700px'],
        content: '<?php echo url("Panel/add_role"); ?>?id='+id //iframe的url
      });
    }
    //删除角色
    function del(id){
      //询问框
        layer.confirm('确认要删除吗?', {
          ...
          //提交数据，跳转页面
          $.post("<?php echo url('Panel/delete'); ?>",{'id':id},function(res){
            if(res.code>0){
                layer.alert(res.msg,{icon:2});
            }else{
                layer.msg(res.msg);
