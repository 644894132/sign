<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:95:"D:\phpstudy\PHPTutorial\WWW\sign\public/../application/admin\view\meeting\person_sign_list.html";i:1572574537;s:72:"D:\phpstudy\PHPTutorial\WWW\sign\application\admin\view\common\head.html";i:1570699778;s:72:"D:\phpstudy\PHPTutorial\WWW\sign\application\admin\view\common\foot.html";i:1572573889;}*/ ?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8">
    ...
    <table class="layui-table">
      ...
      <thead>
       ...
       <tr>
         ...
         <?php if(!empty($vo['sign_time'])): ?>
         <td><?php echo date('Y-m-d H:i',$vo['sign_time']); ?></td>
         <?php else: ?>
         <td></td>
         <?php endif; ?>
         <td>
          <?php if(empty($vo['sign_status'])): ?>
          <button type="button" class="layui-btn" onclick="sign('<?php echo $vo['number']; ?>','<?php echo $vo['Account']; ?>',this)">签到</button>
          <?php elseif($vo['sign_status'] == 1): ?>
          <button type="button" class='layui-btn' style="background-color:grey;cursor:not-allowed;">已签到</button>      
          <?php endif; if(empty($vo['sign_status'])): ?>
          <button type="button" class='layui-btn' onclick="leave('<?php echo $vo['number']; ?>','<?php echo $vo['Account']; ?>',this)">请假</button>
 ...
 <script type="text/javascript">
    ...
    function remove(id){
      ...
      layer.confirm('确认要删除吗?',{
        ...
        //提交数据，跳转页面
        $.post('<?php echo url("Meeting/delete"); ?>',{'id':id},function(res){
          if(res.code>0){
  ...
  function sign(meeting_id, teacher_account, obj){
    ...
    $.post("<?php echo url('Meeting/save_sign'); ?>",{'meeting_id':meeting_id,'teacher_account':teacher_account},function(res){
          //是否签到
  ...
  function leave(meeting_id, teacher_account, obj){
    ...
    $.post("<?php echo url('Meeting/save_leave'); ?>",{'meeting_id':meeting_id,'teacher_account':teacher_account},function(res){
          /*//是否请假
