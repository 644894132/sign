<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:82:"D:\phpstudy\PHPTutorial\WWW\sign\public/../application/admin\view\label\index.html";i:1572228351;s:72:"D:\phpstudy\PHPTutorial\WWW\sign\application\admin\view\common\head.html";i:1570699778;s:72:"D:\phpstudy\PHPTutorial\WWW\sign\application\admin\view\common\foot.html";i:1571801470;}*/ ?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8">
...
<div class="layui-input-inline">
 <input type="text" class="layui-input" value="<?php echo $data['seek']; ?>" id="seek" style="width:320px;" placeholder="请输入编号/标签名称">
</div>
<button class="layui-btn" style="margin-left:150px;" onclick="searchs()"><i class="layui-icon">&#xe615;</i>搜索</button>
 ...
 <table class="layui-table">
  ...
  <thead>
     <?php if(is_array($data['data']['lists']) || $data['data']['lists'] instanceof \think\Collection || $data['data']['lists'] instanceof \think\Paginator): $key = 0; $__LIST__ = $data['data']['lists'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?>
     <tr>
       <td><?php echo $key; ?></td>
       <td><?php echo $vo['labels']; ?></td>
       <td><?php echo $vo['count']; ?></td>
       <td>
         <button class="layui-btn" onclick="add(<?php echo $vo['id']; ?>)" title="编辑"><i class="layui-icon">&#xe642;</i></button>
         <button class="layui-btn layui-btn-danger" title="删除" onclick="remove('<?php echo $vo['id']; ?>')"><i class="layui-icon">&#xe640;</i></button>
         <button class="layui-btn layui-btn-normal" title="人员指定" onclick="person_assign('<?php echo $vo['id']; ?>')"><i  class="layui-icon">&#xe61f;</i></button>
       </td>
     </tr>
  ...
  //引用layui主键
      layui.use(['layer','laypage'],function(){
       ...
       function searchs(curpage){
        ...
        //有wd就加载至分页后面
        var url ="/admin.php/admin/Label/index?page="+curpage;
        //var_dump(url);
        if(seek){
          //将搜索到的内容拼接至分页后面
          url+='&seek='+seek;
    ...
    //添加
      function add(id){
        ...
        layer.open({
          ...
          area: ['450px', '400px'],
          //URL地址
          content: '/admin.php/admin/Label/add?id='+id //iframe的url  
       });
     }
       /* //删除
      function remove(id){
      ...
      function remove(id){
        ...
        layer.confirm('确认要删除吗?', {
          ...
          //提交数据，跳转页面
          $.post('/admin.php/admin/Label/delete',{'id':id},function(res){
            if(res.code>0){
      ...
      function person_assign(id){
        ...
        let url = "/admin.php/admin/Label/main_interface?id="+id;
        // console.log(url);
        window.location.href = url;
      }
</script>